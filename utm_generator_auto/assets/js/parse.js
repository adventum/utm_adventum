...
function parse(hash){
  $.ajax({
    ...
    success: function (data){
      ...
      $('#dealer_center_name').val(data['dealer_center_name']);
      $('#dealer_center_name').trigger('change');
      $('#campaign_type').val(data['campaign_type']);
      $('#campaign_type').trigger('change');
      $('#auto_brand').val(data['auto_brand']);
      $('#auto_brand').trigger('change');
      $('#auto_model').find('option').remove().end();
      $('#auto_model').append('<option value = "'+ data['auto_model'] +'">'+ data['auto_model'] +'</option>').find('option:first').attr("selected","selected");
